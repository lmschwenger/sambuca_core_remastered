...
if __name__ == "__main__":
    ...
    # Define SIOP directory path
    siop_dir = os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))), "data", "siops")

    # Plot all SIOP categories
    plot_all_siop_categories(siop_dir, output_dir="./siop_plots")

    # Example of plotting specific SIOPs from different subdirectories
    absorption_siops = [
        ('absorption', 'water_absorption.csv'),
        ('absorption', 'phytoplankton_absorption.csv')
    ]

    substrate_siops = [
        ('substrates', 'sand_substrate.csv'),
        ('substrates', 'seagrass_substrate.csv')
    ]

    # Plot water and phytoplankton absorption
    plot_siops(absorption_siops, siop_dir,
               output_file="water_phyto_absorption.png",
               title="Water and Phytoplankton Absorption",
               y_label="Absorption (1/m)")

    # Plot substrates
    plot_siops(substrate_siops, siop_dir,
               output_file="substrates.png",
               title="Substrate Reflectance",
               y_label="Reflectance")

    # You can also use glob patterns to select files
    # Plot all absorption files
    plot_siops(['absorption/*.csv'], siop_dir,
               output_file="all_absorption.png",
               title="All Absorption Coefficients",
               y_label="Absorption (1/m)")

    # Plot all backscatter files
    plot_siops(['backscatter/*.csv'], siop_dir,
               output_file="all_backscatter.png",
               title="All Backscatter Coefficients",
               y_label="Backscatter (1/m)")